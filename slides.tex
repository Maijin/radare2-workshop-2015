...
\begin{frame}[fragile]
  ...
  \center qemu-system-arm -M milestone -m 256 -L . -bios bootrom.bin -mtdblock	mbmloader-1.raw -d in_asm,cpu,exec -nographic -s -S
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Load the whole image or unpack it using bios_extract
	\item Open it using the correct segment and offset
	\item r2 load the whole BIOS image automatically
	\item r2 asrock_p4i65g.bin
	\item [0000000] . asrock_p4i65g.r2
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \center r2 -a 8051 ite_it8502.rom
  \center . ite_it8502.r2
\end{frame}
...
\begin{frame}[fragile]
  ...
  \center r2 -a 8051 ite_it8502.rom
  \center . ite_it8502.r2
  \center run 'des' command now:
\end{frame}
% show demo here (if 'des' without '-d' will be fixed

% or may be show the example from here? http://radare.tv/a/44
